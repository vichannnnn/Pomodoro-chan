...
async def joinroom(ctx: lightbulb.Context):
    ...
    if caller not in yaml_data['Owners']:
    ...
    if not voice_state:
        return await ctx.respond("Connect to a voice channel first.", flags=hikari.MessageFlag.EPHEMERAL)
    ...
    return await ctx.respond(f"Connected to <#{voice_channel_id}>", flags=hikari.MessageFlag.EPHEMERAL)
